...
{
    public class GetRecipeByIdInvalidTokenTest
    {
        public class RegisterRecipeInvalidTokenTest : RecipeBookClassFixture
        {
            ...
            public async Task Error_Invalid_Token(string culture)
            {
                ...
                HttpResponseMessage response = await DoGet(url, token: "TokenInvalid", culture);
            ...
            public async Task Error_Without_Token(string culture)
            {
                ...
                HttpResponseMessage response = await DoGet(url, token: "", culture);
                Assert.Equal(StatusCodes.Status401Unauthorized, (int)response.StatusCode);
