...
namespace RecipeBook.Application.UserCases.Recipe
{
    public class RecipeValidator : AbstractValidator<RequestRecipeJson>
    {
        public RecipeValidator()
        {
            ...
            RuleFor(recipe => recipe.Intructions.Count).GreaterThan(0).WithMessage(ResourceMessagesException.MUST_CONTAIN_AN_INSTRUCTION);
            ...
            RuleForEach(recipe => recipe.Intructions).ChildRules(instructionRule =>
            {
            ...
            RuleFor(recipe => recipe.Intructions)
